...
for i = 1:initialRunlength + sampleRunlength + freeRunlength + plotRunlength % loop for the time stamp
    ...
    if linearNetwork
        if noiselevel == 0 | noiselevel == 0.0 | i > initialRunlength + sampleRunlength %washout initial length
...
if i>initialRunlength
    ...
    observer=old_observer*0.98+internalState*0.02;    %computing observer
...
pc1=FindPrincipalcomponent( slowsignalofreservior);
...
% plot weights
 ...
 for state=1:9
  subplot(3,3,state);
    plot(slowsignalofreservior(state,1:2000));
  title('Observers');
 end
