...
for i =1: initialRunlength + sampleRunlength + freeRunlength + plotRunlength
    ...
    %update totalstate except at input positions  
       ...
       if i > initialRunlength
          ...
          s=[intWM, inWM, ofbWM]*totalstate-controllervector';
             internalState = f(s); 
             count=count+1;
      else
            val= [intWM, inWM, ofbWM];
    ...
    reserviorwithcontroller=[reserviorwithcontroller,internalState];
     ...
     observerwithcontroller=observerwithcontroller*0.98+internalState*0.02;    %computing observer
   ...
   controllervector=controllervector+(principalcomponent(j,:)*transposecontroller(j,:)).*gain(:,j);
     ...
     slowsignalofreserviorwithcontroller=[slowsignalofreserviorwithcontroller,observerwithcontroller]; % adding slow observers 
% 
...
%  
  ...
  figure;
 plot((reserviorwithcontroller(40:43,1:initialRunlength+2000))');
